...
| 프로젝트 | 설명 | 핵심 기술 |
|---|---|---|
| **[CareerPilot](https://github.com/jkwltx177/careerpilot)** | Claude Code를 **취준 전담 AI 컨설턴트**로 만드는 오픈소스 템플릿 (1인) — 채용 전문가 페르소나 · **위키형 파일 메모리**(회사별 status/연대기/규칙 이원화) · **멀티에이전트 워크플로우 3종**(정책 변화 스캔 리서치·적대적 자소서 검토·코테 복기), 실제 대기업 지원 캠페인 한 시즌 운영으로 검증 | Claude Code · Multi-Agent Workflow |
...
| **[TrendPilot](https://github.com/jkwltx177/trendpilot-kbeauty-agent)** | K뷰티 글로벌 진출 전략 B2B 에이전트 (2인, **준우승**) — 제품 사진 1장 → 규제 RAG·트렌드 분석 → 전략 PDF·광고 에셋 | GPT-4o · DALL·E 3 · ChromaDB |
