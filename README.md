...
AI 서비스를 만들 때 가장 오래 붙잡는 질문은 "이게 실제로 맞게 동작하는지 어떻게 아는가"입니다. 그래서 골든셋을 만들어 전수 실측하고, 검증되지 않은 성공을 성공으로 세지 않는 판정 체계까지를 설계 범위로 봅니다. LLM 에이전트와 RAG에서 시작해 MSA 백엔드, K8s 배포까지 한 사람 손으로 이어본 기록이 아래 프로젝트들입니다.
